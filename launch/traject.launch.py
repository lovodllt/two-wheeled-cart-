...
def generate_launch_description():
    ...
    timed_planner = ExecuteProcess(
        cmd=["python3", os.path.join(pkg_path, "scripts", "timed_velocity_planner.py")],
        output="screen"
    )
    ...
    after_generate_urdf1 = RegisterEventHandler(
        event_handler=OnProcessExit(
            ...
            on_exit=[
                ...
                gazebo_launch,
                TimerAction(period=2.0, actions=[spawn_entity_node1])
            ]
        )
    )
    ...
    after_spawn_robot1 = RegisterEventHandler(
        event_handler=OnProcessExit(
            ...
            on_exit=[
                controller_manager1,
                TimerAction(period=3.0, actions=[joint_state_broadcaster1, diff_drive_controller1])
            ]
        )
    )

    # 控制器启动后，延迟5秒启动自定义规划器（确保小车初始化完成）
    after_start_controllers = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=diff_drive_controller1,
            on_exit=[TimerAction(period=5.0, actions=[timed_planner])]
        )
    )
    ...
    launch_description = LaunchDescription([
        generate_urdf1,
        after_generate_urdf1,
        after_spawn_robot1,
        after_start_controllers
    ])
